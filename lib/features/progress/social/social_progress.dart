...
class SocialSkillSelectionChart extends ConsumerWidget {
  ...
  Future<void> savePDF(Kid kid) async {
    ...
    try {
      ...
      final file = File("${directory?.path}/ProgresDezvoltareSocială.pdf");
  ...
  void extractSkillsForPDF(BuildContext context, Kid kid) async {
    await savePDF(kid);
    final directory = await getExternalStorageDirectory();
    String filePath = '${directory?.path}/ProgresDezvoltareSocială.pdf';
      ...
      if (context.mounted) {
        showDialog(
          ...
          builder: (BuildContext context) {
            return AlertDialog(
              title: const Text('Progres dezvoltare socială'),
